...
## 搜索策略（强制）
- 默认搜索入口：searxng（http://127.0.0.1:8765/search?q=关键词&format=json）。
- 若 searxng 不可用，fallback 到 VPS Codex CLI：/home/ubuntu/.openclaw/workspace/scripts/codex-auto.sh。
- 禁止以缺 Brave API Key作为默认阻断理由。
- 输出必须包含：结论、来源链接、具体日期。
